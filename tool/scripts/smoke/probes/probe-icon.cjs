const { chromium } = require('playwright');
(async () => {
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage({ viewport: { width: 1440, height: 900 } });
  await page.goto('http://localhost:5173/', { waitUntil: 'networkidle', timeout: 60000 });
  await page.waitForTimeout(1200);
  await page.evaluate(() => { const r=document.getElementById('onb-root'); if(r){r.classList.remove('is-open');r.style.display='none';} });
  const info = await page.evaluate(() => {
    const btn = document.getElementById('help-hotspot-btn');
    const icon = btn?.querySelector('.help-hotspot-icon');
    const r = btn?.getBoundingClientRect();
    return {
      size: r ? { w: r.width, h: r.height } : null,
      iconSrcPrefix: (icon?.getAttribute('src')||'').slice(0, 30),
      iconNatural: icon?.naturalWidth,
      bg: btn ? getComputedStyle(btn).backgroundColor : null,
    };
  });
  await page.locator('#help-hotspot-btn').hover({ timeout: 5000 });
  await page.waitForTimeout(400);
  const pop = await page.locator('#help-hotspot-pop').evaluate(el => ({
    open: !el.classList.contains('hidden'),
    gif: (el.querySelector('img')?.getAttribute('src')||'').includes('gif'),
  }));
  await page.screenshot({ path: process.env.OUT + '/icon-hotspot.png', clip: { x: 60, y: 400, width: 400, height: 80 } });
  console.log(JSON.stringify({ info, pop }, null, 2));
  await browser.close();
})().catch((e) => { console.error(e); process.exit(1); });
