...
/* 审核端默认暗色工作台，可切换亮色；注入双端共享 CSS 与审核端独有样式。
...
document.documentElement.dataset.theme = localStorage.getItem(REVIEWER_THEME_KEY) === 'light' ? 'light' : 'dark';
...
const scene = new THREE.Scene(); scene.background = new THREE.Color(0x080909); const camera = new THREE.PerspectiveCamera(45, 1, .01, 1000); camera.position.set(2.8, 2.2, 4.2); const renderer = new THREE.WebGLRenderer({ canvas: $('canvas'), antialias: true }); renderer.setPixelRatio(Math.min(devicePixelRatio, 2)); renderer.outputColorSpace = THREE.SRGBColorSpace; renderer.toneMapping = THREE.ACESFilmicToneMapping; renderer.toneMappingExposure = 0.92; const composer = new EffectComposer(renderer); composer.addPass(new RenderPass(scene, camera)); const outlinePass = new OutlinePass(new THREE.Vector2(1, 1), scene, camera); outlinePass.edgeStrength = 3; outlinePass.edgeGlow = 0; outlinePass.edgeThickness = 1; outlinePass.pulsePeriod = 0; outlinePass.visibleEdgeColor.set(0xffb347); outlinePass.hiddenEdgeColor.set(0x6b3a12); composer.addPass(outlinePass); const controls = new OrbitControls(camera, renderer.domElement); controls.enableDamping = true; controls.dampingFactor = .075; scene.environmentIntensity = 0.55; scene.backgroundIntensity = 0.28; scene.backgroundBlurriness = 0.38; scene.add(new THREE.HemisphereLight(0xffffff, 0x252a31, 0.7)); const key = new THREE.DirectionalLight(0xffffff, 1.25); key.position.set(4, 6, 5); scene.add(key); const fill = new THREE.DirectionalLight(0xb9d5ff, 0.38); fill.position.set(-4, 2, -3); scene.add(fill); const rgbeLoader = new RGBELoader(); const pmremGenerator = new THREE.PMREMGenerator(renderer); pmremGenerator.compileEquirectangularShader(); const hdriSource = globalThis.__AN_HDR_SOURCE__ || '/hdri/brown_photostudio_02_2k.hdr'; rgbeLoader.load(hdriSource, (texture) => { texture.mapping = THREE.EquirectangularReflectionMapping; scene.environment = pmremGenerator.fromEquirectangular(texture).texture; scene.background = texture; pmremGenerator.dispose(); }, undefined, (error) => console.warn('HDRI 加载失败，使用中性灯光回退', error)); const raycaster = new THREE.Raycaster(); const pointer = new THREE.Vector2();
function applyReviewerTheme(theme, persist = true) {
  ...
  scene.background = new THREE.Color(dark ? 0x080909 : 0xe8efe9);
  scene.backgroundIntensity = dark ? 0.28 : 0.45;
  scene.environmentIntensity = dark ? 0.55 : 0.72;
  outlinePass.visibleEdgeColor.set(dark ? 0xffb347 : 0x2f9b6a);
  outlinePass.hiddenEdgeColor.set(dark ? 0x6b3a12 : 0x1a5c3e);
