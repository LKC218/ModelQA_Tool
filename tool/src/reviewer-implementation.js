...
document.addEventListener('keydown', (event) => { if (event.key !== 'Escape') return; if (viewer.isIsolating()) { viewer.clearIsolate(); applyIsolateUI(); refreshTree(); return; } if (state.courseMenuId) { state.courseMenuId = null; renderReviewCourseRail(); } });
