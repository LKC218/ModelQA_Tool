...
function refreshTree() {
  ...
  if (!model) {
    $('nodes').textContent = '—';
    $('tree').innerHTML = emptyState('⌗', '选择模型后显示层级', '导入 GLB 并选中模型；单击选中零件，双击淡化其他');
    return;
  }
  $('nodes').textContent = `${model.nodes.length} 节点`;
  renderTree($('tree'), model.gltf.scene, { selected: state.selected, onSelect: selectNode, onIsolate: isolateFromTree });
...
document.addEventListener('keydown', (event) => { if (event.key !== 'Escape') return; if (viewer.isIsolating()) { viewer.clearIsolate(); applyIsolateUI(); refreshTree(); return; } if (!drawer.classList.contains('hidden')) closeDrawer(); }); $('fit').onclick = fit;
$('wire').onclick = () => { state.wire = !state.wire; viewer.setWireframe(state.wire); $('wire').classList.toggle('active', state.wire); };
$('isolate').onclick = () => { if (!state.selected) return; viewer.toggleIsolate(state.selected); applyIsolateUI(); refreshTree(); };
/* 隔离态：画布锁定选中，仅空白处可退出（ESC/按钮另走）；非隔离态仍可点选 */
renderer.domElement.addEventListener('pointerdown', (event) => { const root = current()?.gltf.scene; if (!root || event.button !== 0) return; const rect = renderer.domElement.getBoundingClientRect(); pointer.set(((event.clientX - rect.left) / rect.width) * 2 - 1, -((event.clientY - rect.top) / rect.height) * 2 + 1); raycaster.setFromCamera(pointer, camera); const hit = raycaster.intersectObject(root, true)[0]; if (viewer.isIsolating()) { if (!hit) { viewer.clearIsolate(); applyIsolateUI(); refreshTree(); } return; } if (hit) selectNode(hit.object); });
