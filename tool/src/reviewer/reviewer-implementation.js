...
/* Bottom Sheet peek 摘要条：half 面板打开时隐藏，收起时常显（≤600 移动形态） */
function updateSheetPeekVisibility() {
  const peek = $('sheet-peek');
  if (!peek) return;
  peek.classList.toggle('hidden', !state.payload || !!document.querySelector('.sidebar.is-open'));
}
function updateSheetPeek(model, modelStatus, issueCount) {
  const peek = $('sheet-peek');
  if (!peek) return;
  peek.classList.toggle('hidden', !state.payload || !!document.querySelector('.sidebar.is-open'));
  if (!model) {
    $('sheet-peek-title').textContent = '未选择模型';
    $('sheet-peek-status').textContent = '待审核';
    $('sheet-peek-status').dataset.status = 'pending';
    $('sheet-peek-count').textContent = '';
    return;
  }
  const statusMeta = STATUS_META[modelStatus] || STATUS_META.pending;
  $('sheet-peek-title').textContent = model.displayName || model.fileName;
  $('sheet-peek-status').textContent = modelStatus === 'pending' ? '待审核' : `${statusMeta.icon} ${statusMeta.short || statusMeta.label}`;
  $('sheet-peek-status').dataset.status = modelStatus;
  $('sheet-peek-count').textContent = issueCount ? `${issueCount} 问题` : '';
}
function setMobilePanel(name) {
  ...
  syncSidebarMask();
  updateSheetPeekVisibility();
}
/* 移动抽屉遮罩：随抽屉开关显隐，点击收起当前抽屉（≤600 生效，DOM 单例惰性创建） */
...
$('sheet-peek')?.addEventListener('click', () => setMobilePanel('right'));
...
function renderReview() {
  ...
  updateSheetPeek(model, modelStatus, issues.length);
...
function reviewerHtmlShell(data) {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,viewport-fit=cover"><title>${PAGE_TITLE_REVIEWED}</title></head><body class="is-reviewer is-app"><div id="app"></div><script>window.__AN_REVIEW_PAYLOAD__=${JSON.stringify(data).replace(/</g, '\\u003c')};</script><script>${embed}</script></body></html>`;
}
...
async function loadDirectory(files, options = {}) {
  ...
  try {
    ...
    $('title').textContent = project.displayTitle || project.name || PAGE_TITLE;
...
renderer.domElement.addEventListener('pointercancel', () => { pickOrigin = null; }); resize(); if (state.payload) { migrate(); $('title').textContent = state.payload.project.displayTitle || state.payload.project.name || PAGE_TITLE; renderModels(); updateSheetPeek(null, 'pending', 0); if (state.payload.project.models?.[0] && !needsFolderFiles()) loadModel(state.payload.project.models[0].modelId); }
refreshFolderDropUI();
if (needsFolderFiles()) $('status').textContent = '手机请优先打开已审 HTML；桌面可拖入或选择审核包文件夹';
restoreLastPackageIfPossible().catch(() => { updateLastDirUI(); });
initReviewerOnboarding({
  autoStart: true,
  mobilePanels: {
    left: () => setMobilePanel('left'),
    right: () => setMobilePanel('right'),
  },
});
