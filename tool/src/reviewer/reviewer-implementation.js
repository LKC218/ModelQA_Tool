...
function reviewerHtmlShell(data) {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0,viewport-fit=cover"><title>${PAGE_TITLE_REVIEWED}</title></head><body><div id="app"></div><script>window.__AN_REVIEW_PAYLOAD__=${JSON.stringify(data).replace(/</g, '\\u003c')};</script><script>${embed}</script></body></html>`;
}
...
renderer.domElement.addEventListener('pointercancel', () => { pickOrigin = null; }); resize(); if (state.payload) { migrate(); $('title').textContent = state.payload.project.displayTitle || state.payload.project.name || PAGE_TITLE; renderModels(); if (state.payload.project.models?.[0] && !needsFolderFiles()) loadModel(state.payload.project.models[0].modelId); }
refreshFolderDropUI();
