...
- [开发者端工作台样式](../tool/src/styles.css)：PC 优先的三栏弹性布局、顶部横向课程卡片、两行课程标题、边界感知的左右导航按钮、模型导航浮层、模型层级树（缩进连线/类型图标/Empty 徽标/选中左条/空态引导/面包屑）、全局交互动效、滚动条和课程/模型/节点选中态。
- [审核者离线运行时](../tool/src/reviewer-entry.js)：正式运行时入口；委托给 `reviewer-implementation.js`，负责课程树、模型独立审核、模型/零件 Issue、问题定位描边和审核结果导出。
- [审核者离线实现](../tool/src/reviewer-implementation.js)：自包含审核端的顶部课程卡片、模型快速导航、`byModel` 问题保存、节点定位、保留贴图颜色的局部描边、与开发端同步的模型层级树视觉、离线目录加载及 PC 工作台排版。
- [审核运行时构建脚本](../tool/scripts/build-reviewer.mjs)：把审核端依赖与 Penguin Museum HDR 数据打入固定离线运行时模板。
